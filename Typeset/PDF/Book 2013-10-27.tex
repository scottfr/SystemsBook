...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-01.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-02.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-03.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-04.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-05.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-06.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-07.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-08.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-09.png}
...
\begin{figure}[htbp]
\centering
\includegraphics{00-intro-10.png}
...
Did you realize that while the structure demonstrates a property of
growth, growth is not a property you can find in any of the individual
components? This is what is called an emergent property. All models have
one or more emergent properties. Have you ever heard the statement ``The
whole is greater than the sum of its parts''? This is an example of what
...
Each of the accumlations in the simulation changes in a different way.
The timeframes of concern are also different (timeframe being the time
it takes for a noticeable change in the accumlation).
...
are the values for those stocks, e.g., gallons, pounds, kilograms, etc.?
What are the flows that increase or decrease those stocks? What are the
timeframes over which you think about the increase or decrease of the
stock?

\completeEx 

``Essence Property \# 1'' presented the concept of a stock that changes
based on flows in and flows out. The manner in which the stock changes
is independent of the stock itself. The ``Essence Property \#
...
The emergent property of this model is exponential growth; a property
that cannot be found in any of the elements of the model when separated
from the model. Get used to looking for and identifying emergent
properties. You should come to realize they are very important.
...
situations you are familiar with that demonstrate exponential growth.
What are the values for the stocks and flows in those situations? What
are the timeframes for the exponential growth? Once it starts why
doesn't the growth continue forever?

\completeEx 

Exponential growth results from reinforcing feedback from a stock to a
flow. The ``Essence Property \# 3''\footnote{Essence Property \# 3
  \url{http://insightmaker.com/insight/6120}.} model will introduce a
...
As in the previous exercise, take a few minutes and identify a number of
situations you are familiar with that demonstrate goal seeking behavior.
What are the values for the stocks and flows in those situations? What
defines the goal for these situations? What governs the timeframe over
which the goal is achieved?

\completeEx 

A goal seeking structure pursues the goal in a manner that depends on
balancing feedback. Every goal seeking structure has at least one
balancing feedback and every balancing structure is tending toward a
goal. Goal seeking is the emergent property of the structure that can't
be found in any of the elements of the structure.
...
future. These three structures are the building blocks from which
everything else is created. No matter how complex or complicated a model
may seem, it's simply some number of these three structures interacting.
These three structures are presented together in the ``Similar
Structures / Different Behavior''\footnote{Similar Structures /
  Different Behavior \url{http://insightmaker.com/insight/5138}.} model
...
Hopefully you have become comfortable with these three structures, as
you will experience them over and over. If you're not comfortable please
interact with the previous models until you are.

\paragraph{}

\beginEx{Exercise 1-4}

The best way to determine the extent to which you understand these three
structures is to explain them to someone else. You might want to
actually use the models presented to achieve this.

\completeEx 

\section{Three Types of Models}

In previous sections you interacted with two different kinds of modelsin
all the simulation models you ran: qualitative models (no numbers -
``Follow the Clues'' and ``Bird Feeder Dilemma'') and quantitative
models (with numbers). In the ``Three Types of Models''\footnote{Three
...
Diagrams. We will also provide more detail about quantitative Stock \&
...
The type of model you develop depends on what you're trying to
...
simulation model, and finally use a causal loop diagram to present to
others the insights identified. It's of critical importance to consider
the intent and your audience when you set out to create a model. That's
not to say the intent may not change along the way. The development of
models should be considered, above all else, as a learning process.
...
In the {[}\textbf{Stop at Grandmas}{]} variable change \{0 miles\} to
\{0 kilometers\}. Does the model still work? Why?

\hfill{} \hyperref[Ans-2-1]{Answer Available}

\completeEx 

Seldom is there ever just one way to build a model. You build the model
to help you understand something and you might do that in different
ways. Even a model as simple as Going to Grandma's can be structured in
several different ways other than starting with a stock of 4.5 and
reducing it by walking.

\hyperdef{}{Ex-2-2}{\paragraph{}\label{Ex-2-2}}

\beginEx{Exercise 2-2}

Go to Insight Maker and try to build one or two alternatives to this
model.
...
Hopefully the Going to Grandma's has reinforced the build a little, test
a little approach for developing models. The introduction to using units
should have provided a sense of why they can be so useful. Oh, and don't
forget about putting notes in your models. Wiring diagrams without
knowing what the pieces mean are generally not very useful.
...
Note that in this model you might have considered the Workers as a stock
as they are actually a collection. The reason they're not considered as
a stock is that the number remains constant in the context of this
particular model. In a different model workers might actually be a stock
with an inflow and and outflow.

\paragraph{}

\beginEx{Exercise 2-3}

Set up the above model to run with Time Step of 0.25. Compare the
results of this run with the results of the previous run above. By
making the time step smaller have we improved the accuracy of result?
Why?

\completeEx 

The appropriate Time Step is one that captures the activity occurring
within the model. In this case the Workers are in integers and Project
Work in days, both of which are in integers, and with the Time Units in
days the appropriate Time Step would seem to be 1. Though as it turns
out the appropriate Time Step is 0.5. If there were events which
...
wrong or your assumptions are wrong. Each case represents an opportunity
...
this a bit strict though after you add several elements to a model,
simply to find it doesn't work, and you spend hours trying to figure out
why, you may have a better appreciation for this guideline.
...
It is often the case that when things are going just the way we want
them to we tend to stop paying attention to them. Experience has been
telling us for a long time this is not a good idea. Are we learning from
the experience? The ``Maintaining Personnel Resources''\footnote{Maintaining
  Personnel Resources \url{http://insightmaker.com/insight/162}.}
simulation provides insights into a situation where an established
policy for hiring new employees won't suffice in the face of other
changes.
...
Have you ever pursued a goal and found that you actually overshot the
goal and had to back up to get back to the goal? The ``Balancing Loop
with Delay''\footnote{Balancing Loop with Delay
  \url{http://insightmaker.com/insight/133}.} model is a variation of
the standard Balancing Loop. The variation being that there are one or
more delays in the structure which are responsible for producing, as
...
You might ask how could it be that it might take 4 days for someone to
get a sense of what the results of the previous actions were, and that
...
this structure operates in this manner whether the time units are hours,
minutes, seconds or microseconds.

\hyperdef{}{Ex-2-6}{\paragraph{}\label{Ex-2-6}}

\beginEx{Exercise 2-6}

How should one deal with the delay in this structure so effort doesn't
have to be expended to correct the situation after the goal is exceeded?

\hfill{} \hyperref[Ans-2-6]{Answer Available}
...
The ``Infinite Drinkers''\footnote{Infinite Drinkers
  \url{http://insightmaker.com/insight/9776}.} is an attempt at modeling
...
and at times they can be really simple. Often the most amazing insights
arise from what appears to be very simple models. The more complex the
model the more likely insights are apt to get lost in the detail.
...
\begin{figure}[htbp]
\centering
\includegraphics{03-im-9731.png}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_49/diagram_11.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_49/result_13.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_49/result_19.png}
\end{minipage}




\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_49/result_20.png}
\end{minipage}
...
\item \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_49/data_25.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_50/diagram_13.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_50/result_17.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_50/result_20.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_50/result_23.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_50/result_26.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_51/diagram_7.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_51/result_19.png}
\end{minipage}
...
These are powerful techniques that can be used to draw definitive
conclusions about the behavior of a system. These conclusions can
supplement your simulation work to generate a comprehensive analysis of
a dynamic system.
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_52/diagram_4.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_52/result_6.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_52/diagram_13.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_52/result_23.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_52/diagram_29.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_52/result_34.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_53/diagram_6.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_53/diagram_13.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_53/result_21.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_53/result_24.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_54/diagram_3.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_54/diagram_11.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_54/result_13.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_54/result_15.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_54/result_18.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_55/diagram_3.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_55/diagram_8.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_55/result_15.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_55/result_21.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_55/result_23.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_56/diagram_3.png}
\end{minipage}
...
\item The model diagram should now look something like this: \par \begin{minipage}{\linewidth}  \centering \includegraphics{model_56/diagram_10.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_56/result_17.png}
\end{minipage}
...
\item Run the model. Here are sample results:\par \begin{minipage}{\linewidth}  \centering \includegraphics{model_56/result_20.png}
\end{minipage}
...
\includegraphics{180.png}Pedro Almaguer, \includegraphics{181.png}Lorry
Antonucci, \includegraphics{182.png}Hugh J Campbell Hr,
\includegraphics{183.png}Jamie Chapman, \includegraphics{184.png}Stephen
Hobbs, \includegraphics{185.png}Lise Inman,
\includegraphics{186.png}Justin Lyon, \includegraphics{187.png}Mike
Parker, \includegraphics{188.png}David Patterson,
\includegraphics{189.png}Anand Rao, \includegraphics{190.png}Matt
Sadinsky, \includegraphics{191.png}Ken Shepard,
\includegraphics{192.png}Janet Singer, \includegraphics{193.png}Raji
Sirajuddin, \includegraphics{194.png}Ben Taylor,
\includegraphics{195.png}Raid M Zaini
